...
import {
  ...
  CheckCircle2, Loader2, Share2, Dumbbell, Play,
} from 'lucide-react'
...
export function ExecucaoClient({ alunoId, sessao, ciclo }: { alunoId: string; sessao: Sessao; ciclo: Ciclo }) {
  ...
  // Unix ms when the session started (or adjusted after resume)
  const [sessionStartTime, setSessionStartTime] = useState<number | null>(null)
  // Unix ms when paused (null = running)
  ...
  const sessionSecs = pausedAtMs !== null && sessionStartTime !== null
    ? Math.floor((pausedAtMs - sessionStartTime) / 1000)
  ...
  // Items the user manually expanded after they were auto-collapsed (done)
  const [expandedDoneItems, setExpandedDoneItems] = useState<Set<string>>(new Set())
    ...
    async function initSession() {
      ...
      if (existing) {
        ...
        const { data } = await (supabase as any)
          .from('workout_sessions')
          .insert({ aluno_id: alunoId, sessao_id: sessao.id, iniciado_em: now, status: 'em_andamento' })
          .select('id')
          .single()

        if (data) {
          setWorkoutSessionId((data as any).id)
  ...
  function persistSerie(itemId: string, serieNum: number, concluida: boolean, carga: string) {
    if (!workoutSessionId) return
    const cargaNum = parseFloat(carga.replace(',', '.')) || null
    ;(supabase as any).from('set_executions').upsert({
      session_id: workoutSessionId,
      sessao_item_id: itemId,
      numero_serie: serieNum,
      concluida,
      carga_registrada: cargaNum,
    }, { onConflict: 'session_id,sessao_item_id,numero_serie' }).then(() => {})
  }

  function persistCarga(newCarga: Record<string, string>) {
    if (!workoutSessionId) return
    ;(supabase as any).from('workout_sessions')
      .update({ carga_json: newCarga })
  ...
  function toggleSerie(itemId: string, serieNum: number, totalSeries: number) {
    const prevSet = seriesDone[itemId] ?? new Set<number>()
    const newSet = new Set(prevSet)
    const wasDone = newSet.has(serieNum)
    wasDone ? newSet.delete(serieNum) : newSet.add(serieNum)
    const allDone = newSet.size === totalSeries

    setSeriesDone(prev => ({ ...prev, [itemId]: newSet }))
  ...
  function toggleExerciseDone(itemId: string, totalSeries?: number) {
    const nowDone = !exercisesDone.has(itemId)

    setExercisesDone(prev => { const s = new Set(prev); nowDone ? s.add(itemId) : s.delete(itemId); return s })

    if (totalSeries) {
      const newSeriesSet = nowDone
        ? new Set(Array.from({ length: totalSeries }, (_, i) => i + 1))
        : new Set<number>()
      setSeriesDone(prev => ({ ...prev, [itemId]: newSeriesSet }))

      const carga = cargaRegistrada[itemId] ?? ''
      for (let s = 1; s <= totalSeries; s++) {
        persistSerie(itemId, s, nowDone, carga)
      }
    }
  ...
  function handleCargaChange(itemId: string, value: string) {
    const newCarga = { ...cargaRegistrada, [itemId]: value }
    setCargaRegistrada(newCarga)
  }

  function handleCargaBlur(itemId: string, value: string) {
    const newCarga = { ...cargaRegistrada, [itemId]: value }
    setCargaRegistrada(newCarga)
    persistCarga(newCarga)
  }

  // ── Resume (retomar after Sair e Pausar) ───────────────────────────────────
  ...
  function sairSemPausar() { setShowExitModal(false); router.back() }

  // ── Finalize ───────────────────────────────────────────────────────────────
  function calcVolume() {
    return itens.reduce((total, item) => {
      if (!exercisesDone.has(item.id)) return total
      const carga = parseFloat(cargaRegistrada[item.id]?.replace(',', '.') || '0') || (item.carga_kg ?? 0)
  ...
  async function enviarFeedback() {
    ...
    try {
      ...
      await (supabase as any).from('workout_feedbacks').insert({
  ...
  function drawShareCard(canvas: HTMLCanvasElement) {
    ...
    ctx.beginPath(); ctx.arc(W / 2, 155, 52, 0, Math.PI * 2); ctx.fillStyle = '#14532D'; ctx.fill()
    ...
    const statsY = 570
    ctx.fillStyle = 'rgba(15,30,50,0.82)'; ctx.fillRect(40, statsY-30, W-80, 80)
  ...
  function renderItem(item: SessaoItem) {
    ...
    // Collapsed when done, unless user explicitly expanded it
    const isCollapsed = isDone && !expandedDoneItems.has(item.id)
    ...
    if (isCollapsed) {
      return (
        <button
          key={item.id}
          onClick={() => setExpandedDoneItems(prev => { const n = new Set(prev); n.add(item.id); return n })}
          className="w-full p-4 flex items-center gap-3 text-left"
        >
          <CheckCircle2 size={20} className="text-green-500 fill-green-50 flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-secondary text-sm truncate">{ex?.nome ?? '–'}</p>
            {cargaRegistrada[item.id] && (
              <p className="text-xs text-outline truncate">Carga: {cargaRegistrada[item.id]}</p>
            )}
          </div>
          <span className="text-xs text-green-600 font-semibold flex-shrink-0">feito ✓</span>
        </button>
      )
    }
    ...
    return (
      <div key={item.id} className={`p-4 ${isDone ? 'bg-green-50/40' : ''}`}>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <input
            type="text"
            className="flex-1 max-w-xs border border-outline-variant rounded-lg px-2 py-1 text-sm focus:outline-none focus:border-primary"
            placeholder={carga ? String(carga) : 'ex: 40/45/50 ou 14kg cada lado'}
            value={cargaRegistrada[item.id] ?? ''}
            onChange={e => handleCargaChange(item.id, e.target.value)}
            onBlur={e => handleCargaBlur(item.id, e.target.value)}
          />
        </div>
        ...
        {item.observacoes && (
          <div>
            ...
            <p className="text-sm text-outline leading-snug whitespace-pre-line">{item.observacoes}</p>
          </div>
        )}
      </div>
    )
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <p className="text-sm text-outline">Carregando treino...</p>
      </div>
    )
  }
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden">
            <div className="p-5 border-b border-outline-variant text-center">
              ...
              <p className="text-sm text-outline mt-1">O progresso está salvo e pode ser retomado.</p>
            </div>
            <div className="p-3 space-y-2">
              <button onClick={sairEPausar} className="w-full py-3 px-4 rounded-xl bg-secondary text-white font-semibold text-sm">
                Sair e pausar o timer
              </button>
              <button onClick={sairSemPausar} className="w-full py-3 px-4 rounded-xl bg-gray-100 text-secondary font-semibold text-sm">
                Sair sem pausar
              </button>
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-outline-variant shadow-sm">
        <div className="flex items-center gap-2 px-4 pt-3 pb-2 max-w-2xl mx-auto">
          ...
          <div className="flex-shrink-0 text-right">
            {isPaused ? (
              <button onClick={retomar} className="flex items-center gap-1.5 text-xs font-bold text-orange-600 bg-orange-50 px-3 py-1.5 rounded-xl">
        ...
        {isPaused && (
          <div className="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 flex items-center gap-3">
            <div className="flex-1">
              <p className="text-sm font-bold text-orange-700">Treino pausado</p>
              <p className="text-xs text-orange-600">Tempo registrado: {fmt(sessionSecs)} — toque Retomar para continuar.</p>
            </div>
            <button onClick={retomar} className="flex items-center gap-1.5 text-sm font-bold text-white bg-orange-500 px-4 py-2 rounded-xl hover:bg-orange-600">
        ...
        <div className="space-y-3">
          {groups.map(group => {
            if (group.length === 1) {
              ...
              const isCollapsed = isDone && !expandedDoneItems.has(item.id)
              return (
                <div key={item.id} className={`bg-white rounded-2xl overflow-hidden shadow-card ${restTimer?.itemId === item.id && !isCollapsed ? 'ring-1 ring-orange-300' : ''}`}>
                  {renderItem(item)}
                </div>
              )
            }
